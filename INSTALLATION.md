...
```bash
curl -LO https://github.com/Lynn-Lee/DataFusionX-Deploy/raw/main/releases/v3.0.1/DataFusionX-Enterprise-v3.0.1.tar.gz
curl -LO https://github.com/Lynn-Lee/DataFusionX-Deploy/raw/main/releases/v3.0.1/DataFusionX-Enterprise-v3.0.1.tar.gz.sha256
shasum -a 256 -c DataFusionX-Enterprise-v3.0.1.tar.gz.sha256
tar -xzf DataFusionX-Enterprise-v3.0.1.tar.gz
cd DataFusionX-Enterprise-v3.0.1
```

如果系统没有 `shasum`，可使用：

```bash
sha256sum -c DataFusionX-Enterprise-v3.0.1.tar.gz.sha256
```
...
| 配置项 | 必填 | 说明 |
| --- | --- | --- |
| `DATAFUSIONX_VERSION` | 是 | 当前部署版本，默认 `3.0.1`。 |
...
```bash
docker load -i datafusionx-backend-commercial-3.0.1.tar
docker load -i datafusionx-frontend-commercial-3.0.1.tar
...
image:
  backend: "ghcr.io/lynn-lee/datafusionx-backend:3.0.1"
  frontend: "ghcr.io/lynn-lee/datafusionx-frontend:3.0.1"
secrets:
